...
{
    window.robotstxtChanged = false;

    jQuery( 'textarea#robotstxt' ).on( 'change keyup paste', function()
    {
        if ( window.robotstxtChanged ) return;

        jQuery( '#toolbar-refresh > button' ).attr( 'disabled', 'disabled' );
    });
 ...
 echo json_encode( $this->robotstxt_default ); ?> )
        ...
        else if ( task == 'robotstxt.validate' )
        {
            alert( <?php
 echo json_encode( JText::_( 'AIMY_SM_MSG_FEATURE_PRO_ONLY' ) ); ?> );
