...
jQuery(document).ready(function(){
 ...
 echo JSession::getFormToken(); ?>', '#ping' );
            ...
            var checkDone; checkDone = function()
            {
                if ( jQuery( 'div.notify-task' ).length ==
                     jQuery( 'div.notify-task[data-done="1"]' ).length )
                {
                    jQuery( '#notify-hint > p' ).fadeOut( 1200 );
                }
                else
                {
                    window.setTimeout( checkDone, 1000 );
                }
            };
