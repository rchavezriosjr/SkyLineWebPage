<?php
/**
 * @version   $Id: ListStrategy.php 30615 2017-07-28 09:36:17Z matias $
...
class RokBooster_Joomla_ListStrategy extends RokBooster_Joomla_AbstractStrategy
{
	...
	protected function identifyScriptFiles()
	{
		...
		foreach ($doc->_scripts as $filename => $fileinfo) {
			...
			$mime = isset($fileinfo['mime']) ? $fileinfo['mime'] : (isset($fileinfo['type']) ? $fileinfo['type'] : 'text/javascript');
			$file->setMime($mime);
			$file->setType('js');
			if (isset($fileinfo['defer'])) $file->addAttribute('defer', 'defer');
			if (isset($fileinfo['async'])) $file->addAttribute('async', 'async');
	...
	protected function identifyStyleFiles()
	{
		...
		foreach ($doc->_styleSheets as $filename => $fileinfo) {
			...
			$file->setType('css');
			$file->setMime($mime);
			if (isset($fileinfo['attribs'])) {
				$file->setAttributes($fileinfo['attribs']);
			}
			if (isset($fileinfo['media'])) {
